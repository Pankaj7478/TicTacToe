export const evaluatebar=(i,j,seti,setj,map,count,data,rdata,arr,lock)=>{
    // console.log("lock", lock);
    // let t0=arr[0],t2=arr[2],t4=arr[4];
    // let t1=arr[1],t3=arr[3],t5=arr[5];
    // if(t1>t3&&t3>t5)[t1,t3,t5]=[t5,t3,t1];
    if(lock===true&&rdata[0][2]===2)seti(3);
    if(lock===true&&rdata[0][2]===1)seti(4);
    // seti(4);
    if(count===1)seti(0);
    if(count===2){
        ...
        console.log(t1,t2,1,"1");
        if((t1===1&&t2===7)||(t1===3&&t2===5)){seti(2);setj(5);}
        else if((t1===0&&t2===2||t1===0&&t2===6||t1===2&&t2===8||t1===6&&t2===8)&&arr[1]!=4){}
        else seti(0);
    }
    else if(count===4){
        if(data[0]===data[8]&&map[rdata[0][1]]===data[8]&&(data[4]&&(data[2]||data[6]))){seti(1);setj(3);}
        else if(data[2]===data[6]&&map[rdata[0][1]]===data[6]&&(data[4]&&(data[0]||data[8]))){seti(1);setj(3);}
        if(arr[0]===4){
